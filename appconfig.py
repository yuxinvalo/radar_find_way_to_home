...
def basic_radar_config():
    basicRadarConfig = {
        ...
        "bscanRefreshInterval": 1000,  # 灰度图刷新频率，每收到500条数据刷新一次
        "receiveFreq": 0.01,  # 读取雷达数据的频率，每n秒收一次， 最好0.01秒，如果出现卡顿，可以放到0.02-0.03, 此数据和画雷达图的刷新时间共享
        "receiveFreqMocks": 0.02,  # 读取模拟雷达数据的频率， 每N秒读取一次，最好是0.02， 此数据和雷达图的刷新时间共享
        "calculateFreq": 0.08,  # 计算Feats的频率，每n秒计算一次， 最好设置为 读取雷达数据频率*5， 这样可以同步，并减少线程工作量， 若出现卡顿，可以放慢一点
...
def basic_gps_config():
    basicGPSConfig = {
        ...
        "gpsGraphRefreshInterval": 3,  # 刷新GPS图的频率，2-5秒尚可，如果低于2秒，
                                       # 提醒一波：这是个随着点数增加所用时间也增加的量，容易导致卡死, 可以在界面设置
    }
