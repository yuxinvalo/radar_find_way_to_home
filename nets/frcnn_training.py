...
class Generator(object):
    def __init__(self, bbox_util,
                 train_lines, num_classes,solid,solid_shape=[416,416]):#solid_shape=[600,600]
...
#一个图上的所有anchor
                ...
                # 计算anchor与最大iou的真实框之间的修正参数（即应该有的预测结果）
                assignment = self.bbox_util.assign_boxes(y,anchors) #assignment = anchor_num*5

                num_regions = 256#正负样本的数量可能相差很大 限制两者之和为256
                ...
                yield preprocess_input(np.expand_dims(tmp_inp,0)), tmp_targets, np.expand_dims(y,0)#tmp_targets建议框应该有的预测结果
