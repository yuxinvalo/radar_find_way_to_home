...
def calc_iou(R, config, all_boxes, width, height, num_classes):
    ...
    '''
    # print(all_boxes)
    bboxes = all_boxes[:,:4]#真实框
    gta = np.zeros((len(bboxes), 4))
    for bbox_num, bbox in enumerate(bboxes): #真实框缩放到featuremap上
    ...
    IoUs = []
    # print(gta)
    for ix in range(R.shape[0]): #建议框缩放到featuremap上
    ...
    X = np.array(x_roi)#rpn建议框
    # print(X)
    Y1 = np.array(y_class_num)
    Y2 = np.concatenate([np.array(y_class_regr_label),np.array(y_class_regr_coords)],axis=1) #classifier修正参数
