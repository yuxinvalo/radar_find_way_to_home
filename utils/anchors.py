...
def shift(shape, anchors, stride=config.rpn_stride):
    ...
    return shifted_anchors #所有先验框的个数以及位置

def get_anchors(shape,width,height):#一张图里面的所有anchor
    ...
    network_anchors = np.clip(network_anchors,0,1)#如果中心点在边界的话 先验框会超出
