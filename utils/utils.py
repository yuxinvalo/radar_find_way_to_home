...
class BBoxUtility(object):
    ...
    def encode_box(self, box, return_iou=True):
        '''
        找到与该box的iou>0.7的anchor 并将这些anchor与真实框之间的
        修正系数求解出来存储到encoded_box中 encoded_box=anchor_num*（ 4 + return_iou）
        '''
        iou = self.iou(box) #计算该box与所有anchor的iou
        ...
        # 找到iou>0.7的先验框 并将其存储到assigned_priors
        assigned_priors = self.priors[assign_mask]
        # 逆向编码，将真实框转化为FasterRCNN预测结果的格式 xywh
        # 先计算真实框的中心与长宽
        ...
        # 逆向求取FasterRCNN应该有的预测结果 需要回归的修正参数
        encoded_box[:, :2][assign_mask] = box_center - assigned_priors_center
        encoded_box[:, :2][assign_mask] /= assigned_priors_wh
        encoded_box[:, :2][assign_mask] *= 4

        encoded_box[:, 2:4][assign_mask] = np.log(box_wh / assigned_priors_wh)
        encoded_box[:, 2:4][assign_mask] *= 4 #系数
        return encoded_box.ravel() #anchor_num*（4 + return_iou）

    def ignore_box(self, box): #一个box真实框与所有anchor的重合度
        '''
        计算该box与所有anchor介于0.3-0.7的iou 并存储在ignored_box中
        '''
        iou = self.iou(box)
        ...
        return ignored_box.ravel() #类似flatten 返回的是iou


    def assign_boxes(self, boxes, anchors):
        '''
        屏蔽所有anchor中与所有真实框iou介于0.3-0.7的anchor
        计算anchor与最大iou真实框之间的修正参数 并存储assignment
        返回assignment anchor_num*5
        '''
        self.num_priors = len(anchors)
        self.priors = anchors
        assignment = np.zeros((self.num_priors, 4 + 1))#坐标和是否有目标

        assignment[:, 4] = 0.0
        if len(boxes) == 0:
            return assignment
            
        # 对每一个真实框都进行所有anchor的iou计算（只计算0.3-0.7的iou） 返回真实框_num*先验框_num 存储的是0.3-0.7的iou
        ingored_boxes = np.apply_along_axis(self.ignore_box, 1, boxes[:, :4])
        # 取重合程度最大的先验框，并且获取这个先验框的index
        ingored_boxes = ingored_boxes.reshape(-1, self.num_priors, 1) #返回真实框_num*先验框_num*1
        #单个anchor与所有真实框最大的iou（0.3-0.7之间的）是ignore_iou 1*先验框_num
        ignore_iou = ingored_boxes[:, :, 0].max(axis=0)
        # (1*num_priors)
        ignore_iou_mask = ignore_iou > 0

        assignment[:, 4][ignore_iou_mask] = -1 #去除0.3-0.7的iou的先验框

        # (n, num_priors, 5) 求解与真实框iou>0.7的anchor的修正参数
        encoded_boxes = np.apply_along_axis(self.encode_box, 1, boxes[:, :4])
        # 每一个真实框的编码后的值，和iou
        # (n, num_priors)
        encoded_boxes = encoded_boxes.reshape(-1, self.num_priors, 5)

        # 取出每一个anchor中与真实框最大的iou并且获取这个先验框的index
        # (1*num_priors)
        best_iou = encoded_boxes[:, :, -1].max(axis=0)
        # (1*num_priors)这个先验框与哪个真实框iou最大
...
#assignment中存储anchor与最大IOU真实框之间的修正参数
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox):
        ...
        # 真实框距离先验框中心的xy轴偏移情况 调整后的先验框中心点位置
    ...
    def detection_out(self, predictions, mbox_priorbox, num_classes, keep_top_k=300,
        ...
        '''
        输入是rpn的输出（全部anchor的修正参数）
        输出nms结果（框的坐标是左上右下）
        '''
        # 网络预测的结果
        # 置信度
        mbox_conf = predictions[0] #x_class
        mbox_loc = predictions[1] #x_regression
        # 先验框
        mbox_priorbox = mbox_priorbox#全部anchor
        results = []
# 对每一个图片进行处理
        for i in range(len(mbox_loc)):
            results.append([])
            decode_bbox = self.decode_boxes(mbox_loc[i], mbox_priorbox)
            for c in range(num_classes): #NMS
